...
export const Input: React.FC<InputProps> = ({ label, error, as = 'input', className = '', ...props }) => {
  ...
  return (
    <div className="space-y-2 w-full">
      ...
      <Component
        className={`w-full bg-zinc-900/50 border-2 ${error ? 'border-red-500' : 'border-zinc-800/50'} focus:border-loki-primary/30 focus:bg-zinc-900 rounded-2xl px-5 py-3.5 text-zinc-100 outline-none transition-all placeholder:text-zinc-700 text-base font-medium ${className}`}
        {...props}
      />
